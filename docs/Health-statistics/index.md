...
下面两块教材的 19 章里没有单独列章（第 14 讲《信息技术基础》出现过 Kappa 检验），
但临床上用得很频繁，本站把它们补成了专题，写法和其他各章一致：

| 专题 | 解决什么问题 |
| --- | --- |
| [诊断试验评价（ROC 与 AUC）](./diagnostic-test) | 一个诊断指标准不准？阈值定在哪？为什么同样的试验换个科室就不准了 |
| [一致性信度（Kappa 与 ICC）](./agreement-reliability) | 两位医生、两台仪器测同一批对象，结果一不一致？为什么相关不能当一致 |
