...
::: tip 为什么答案里能直接看到图
本讲的题都是画图题，光看代码看不出对不对，所以参考答案里把**跑出来的图**一并放上了。
这些图是事先在本机跑好存下来的静态图片（本站不在网页里执行 R），
你照着代码跑一遍，出来的就是它们。
:::
...
`USPersonalExpenditure` 原本是「行=消费项、列=年份」，而 **`barplot()` 是「每列一根柱子」**——
所以直接画出来，**柱子是 5 个年份**（数据里也确实是按年对比，见 `test0902.R` 的标题「每年消费数据对比」），
5 个消费项是每根柱子里叠起来的分段。答案里先 `t()` 转置，列变成消费项，
于是**柱子变成消费项**、**年份变成分组**，再配 `beside = TRUE` 把 5 个年份并排画出来。
:::
...
::: info 关于本页的题目与答案
题目来自 `资料/原题/第九周原题`，答案来自 `资料/答案/第九周答案`（仅修正过其中的错别字）。
原题正文里的「图 1~图 15」已经按题号贴回各题下方（取自 `资料/汇总/第九周——高级绘图_files/figure-html/`，与 原题 docx 里的截图是同一批图，但分辨率更高），参考答案卡片里另附该题答案跑出来的实际图形。
:::

::: tip 想弄懂背后的统计学原理
这一讲是 **base R 的绘图操作手册**；什么数据该配什么图、坐标轴与刻度的陷阱、怎样不误导读者，见《卫生统计学》对应的归纳。
