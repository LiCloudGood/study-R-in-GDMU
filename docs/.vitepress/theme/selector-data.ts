  ...
  "nodes": [
    {
      ...
      "options": [
        ...
        {
          "text": "手上有个诊断指标，想评价它准不准",
          "detail": "有金标准的诊断结论 + 该指标的测量值，比如 s100b 能不能判断脑损伤",
          "next": 142
        },
        {
          "text": "同一批对象重复测/重复判，想看看一不一致",
          "detail": "两位医生看同一批片子、两台仪器测同一批样本",
          "next": 145
        }
      ]
    },
    ...
    },
    {
      ...
      "result": {
        ...
        "pit": [
          "把「不拒绝 H₀」说成「证明总体均数等于该标准值」—— 只能说没找到差别",
    ...
    },
    {
      ...
      "result": {
        ...
        "c": {
          "text": "诊断试验评价（ROC 与 AUC）",
          "link": "/Health-statistics/diagnostic-test"
        }
      }
    },
    {
      ...
      "result": {
        ...
        "read": [
          "`ICC` 点估计**和它的 95% 可信区间**（按区间定档，不是按点估计）",
          "同一批数据的 ICC(2,1) 与 ICC(2,k) 差多少",
          "均方 MSR / MSC / MSE —— MSC 比 MSR 大说明「谁在评」比「评的是谁」影响更大"
        ],
        "pit": [
        ...
        "c": {
          "text": "一致性信度（Kappa 与 ICC）",
          "link": "/Health-statistics/agreement-reliability"
        }
      }
    },
    {
      ...
      "result": {
        ...
        "read": [
          "`Kappa`：扣掉碰巧一致之后的一致性",
          "`z` 与 `p-value`：Kappa 是不是显著大于 0（**不等于 Kappa 够高**）",
          "观察一致率 $P_o$"
        ],
        "pit": [
    ...
    },
    {
      ...
      "result": {
        ...
        "read": [
          "加权 `Kappa`（**务必注明是线性还是平方**）",
          "不加权的 Kappa 一起报，能看出「把等级当无序处理」会低多少",
          "观察一致率"
        ],
        "pit": [
          "不写权重种类 —— 线性与平方能差出一倍",
    ...
    },
    {
      ...
      "result": {
        ...
        "read": [
          "`mean(diff)`：系统偏倚（理想情况接近 0）",
          "一致性界限：偏倚 ± 1.96×差值标准差",
          "散点的形状：有没有随均值变化的趋势（比例偏倚）"
        ],
        "pit": [
