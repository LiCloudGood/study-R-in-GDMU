...
::: info 关于本页的题目与答案
题目来自 `资料/原题/第三周原题`，答案来自 `资料/答案/第三周答案`（仅修正过其中的错别字）。
实验四、五用到的 `scores.csv` 也取自 `资料/原题/第三周原题`。
:::

::: tip 想弄懂背后的统计学原理
这一讲讲**数据框怎么操作**；均数、中位数、标准差、四分位数间距、变异系数各自的适用场合，见《卫生统计学》对应的归纳。

详见 **[《卫生统计学》第 4 章　定量资料的统计描述](/Health-statistics/04-describing-quantitative-data)**。
:::
