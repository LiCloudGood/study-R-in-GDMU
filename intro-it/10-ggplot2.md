...
## 实验题 3：geom_polygon 函数

打开 **test1103.R**，完成下面任务。

1. 在脚本文件中，数据框为中国地图数据，其中数量 `long` 为经度数据，`lat` 为纬度数据，用 `geom_polygon` 绘制中国地图的轮廓。
2. 填充颜色为 `lightblue`，轮廓为红色，不显示坐标。

> 提示：需要把 `group` 属性映射到数据框的 `group` 变量。

::: tip 思路提示：geom_polygon 的 group 是关键
`geom_polygon` 会把同一组里的点**按顺序连成一个封闭多边形**。
一张地图有几十个省/岛屿，如果不对它们分组，所有点会被连成一团乱麻：
...
- `group = group` 告诉 ggplot2「同一个 group 值的点才连在一起」。
- `theme_void()` 是「清空一切主题元素」的快捷写法，
  效果等价于把坐标轴、网格、背景统统设成 `element_blank()`。

> `答案/第十一周答案` 中没有收录 `test1103.R`，且中国地图数据也不在本仓库中，
> 此处仅给出思路提示，不算作参考答案。
:::
...
实验题 7 的完整代码原题已直接在文档中给出。`答案/第十一周答案` 未收录 `test1103.R`。
:::
