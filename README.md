...
```text
public/figures/
├── 09-base-graphics/    # 第 9 讲（高级绘图）的 19 张输出图
└── 10-ggplot2/          # 第 10 讲（ggplot2）的 8 张输出图
```

图片直接来自 `汇总/*_files/figure-html/`（也就是 Knitr 渲染 汇总 时生成的原始图），
**不是重新跑一遍的结果**，所以和 汇总 HTML/PDF 里的图完全一致。

> `AnswerBlock` 会自动给 `/figures/...` 补上部署前缀，
> 但在 Markdown 里手写 `<img>` 时**必须**用 `:src="withBase('...')"`，
> 否则部署到子路径下会 404（`intro-it/9-base-graphics.md` 的图集就是这么写的）。
...
| --- | --- |
| 答案代码 | 85 张卡片，全部都有 |
| 输出图形 | 第 10 讲 6 张卡片共 8 张图；第 9 讲末尾有 19 张图的图集 |
| 文字输出 | 11 张卡片 |
